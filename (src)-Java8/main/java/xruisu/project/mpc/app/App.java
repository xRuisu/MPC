...
public class App extends FXMLVariables {
        ...
        public void initialize() {
                ...
                for (Label label : scanLabels) {
                        NodeHandler.setTextFor(label, "Employee");
                }
                for (TextField field : scanFields) {
                        field.setPromptText("Scans");
                }
                for (Object field : employeeListPane.getChildren()) {
                        NodeHandler.setStyleOn((Node) field,
                                        "-fx-background-color: BLACK; -fx-text-fill: #00fffc; -fx-border-color: #1f1f1f; "
                                                        +
                                                        "-fx-border-radius: 5; -fx-background-radius: 5; -fx-font-family: System; -fx-font-size: 12;");
                }
                for (Object field : scanListPane.getChildren()) {
                        NodeHandler.setStyleOn((Node) field,
                                        "-fx-background-color: BLACK; -fx-text-fill: #00fffc; -fx-border-color: #1f1f1f; "
                                                        +
                                                        "-fx-border-radius: 5; -fx-background-radius: 5; -fx-font-family: System; -fx-font-size: 12;");
        ...
        public void setNotification() {
                if (DataManager.getPrevious().length() == 0) {
                        reportView.setImage(new Image(
                                        getClass().getResource("/assets/images/REPORT-VIEWER/report-viewer-icon256.png")
                                                        .toExternalForm()));
                } else {
                        reportView.setImage(new Image(
        ...
        private void resetFieldsAndLabels() {
                ...
                for (Label label : scanLabels) {
                        label.setText("Employee");
                }
                for (TextField field : employeeFields) {
                        field.setText("");
                }
                for (Label label : valueLabels) {
                        label.setText("");
                }
                for (Label label : rateLabels) {
                        label.setText("");
                }
        }
        ...
        private void onEnable() {
                ...
                scanFields = new ArrayList<>();
                scanLabels = new ArrayList<>();
                rateLabels = new ArrayList<>();
                valueLabels = new ArrayList<>();
                employeeFields = new ArrayList<>();

                for (Object node : scanListPane.getChildren()) {
                        if (node instanceof TextField) {
                                scanFields.add((TextField) node);
                        } else if (node instanceof Label) {
                                scanLabels.add((Label) node);
                        }
                }

                for (Object node : employeeListPane.getChildren()) {
                        if (node instanceof Label) {
                                rateLabels.add((Label) node);
                        } else if (node instanceof TextField) {
                                employeeFields.add((TextField) node);
                        }
                }

                for (Object node : valuesListPane.getChildren()) {
                        if (node instanceof Label) {
                                valueLabels.add((Label) node);
                        }
                }
